...
branch = zeros(Int64, (no_branches, 3))
for ln in branch_data
    id = parse(Int64, ln.first)
    branch[id, L_ID:T_BUS] = [id, ln.second["f_bus"], ln.second["t_bus"]]
end

# generators
gen = zeros(Float64, (no_gen, 2))
# sorted buses in 1:no_buses
for gn in gen_data
    id = parse(Int64, gn.first)
    gen[id, GEN_ID:GEN_BUS] = [id, gn.second["gen_bus"]]
end
