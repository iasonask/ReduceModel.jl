...
function partition(bus_data, branch_data)
    ...
    clustering = kmeans(xy', clusters; maxiter=100, display=:iter)
    scatter(xy[:,1], xy[:,2], marker_z=clustering.assignments, color=:lightrainbow, legend=false)
